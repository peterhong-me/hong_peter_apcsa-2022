    ...
    public void encode(Picture messagePict) {
		...
		for (int row = 0; row < this.getHeight(); row++) {
			...
			for (int col = 0; col < this.getWidth(); col++) {
				...
				int firstR = currPixel.getRed();
				int firstDigitR = firstR / ((int)(Math.pow(10,(int)Math.log(firstR))));
				
				
				int firstG = currPixel.getGreen();
				int firstDigitG = firstG / ((int)(Math.pow(10,(int)Math.log(firstG))));
				
				int firstB = currPixel.getBlue();
				int firstDigitB = firstR / ((int)(Math.pow(10,(int)Math.log(firstB))));
				
				
				 int avg = (firstR + firstB + firstG) / 3;
				...
				if (messagePixel.colorDistance(Color.BLACK) < 50) {
					currPixel.setRed(currPixel.getRed() - currPixel.getRed() %10 + avg);
					currPixel.setBlue(currPixel.getBlue() - currPixel.getBlue() % 2 + (avg+1) %10);
					currPixel.setGreen(currPixel.getGreen() - currPixel.getGreen() %2 + (avg+1) %10);
				} 
    ...
    public Picture decode() {
		...
		for (int row = 0; row < this.getHeight(); row++) {
			for (int col = 0; col < this.getWidth(); col++) {
				...
				int firstR = currPixel.getRed();
				int firstB = currPixel.getBlue();
				int firstG = currPixel.getBlue();
				
				if (firstB == (firstR+1) && firstB == (firstG+1)) {
